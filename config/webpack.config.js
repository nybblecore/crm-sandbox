...
module.exports = function (webpackEnv, isCordova) {
  ...
  shouldUseSourceMap = isEnvProduction && process.env.GENERATE_SOURCEMAP === 'true';
  // Webpack uses `publicPath` to determine where the app is being served from.
  // It requires a trailing slash, or the file assets will get an incorrect path.
  // In development, we always serve from the root. This makes config easier.
  const publicPath = isEnvProduction ? (isCordova ? './' : paths.servedPath) : isEnvDevelopment && isCordova ? './' : '/';
  ...
  return {
    ...
    output: {
      // The build folder.
      path: !isCordova ? paths.appBuild : paths.cordovaBuild,
